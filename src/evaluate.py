...
def evaluate(y_true, y_pred, task_type: str = "regression", X_test: pd.DataFrame = None) -> dict:
    if task_type == "regression":
        ...
        if y_true_type not in ['binary', 'multiclass']:
            return {"Error": "y_true must be label (binary or multiclass), not continuous."}
        ...
        return {
            ...
            "ROC AUC": roc_auc_score(y_true, y_pred) if len(np.unique(y_true)) == 2 else "N/A"
        }
